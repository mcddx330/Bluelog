...
### 4.5. メンションランキング表示におけるデータ取り扱い

この機能は、Blueskyの投稿データからメンションされたユーザーのハンドルを集計し、そのメンション回数に基づいたランキングを生成・表示します。
...
-   **集計方法**:
    ...
    2.  抽出したハンドル名ごとにグループ化し、各ハンドルの出現回数（メンション回数）をカウントします。
    3.  結果は、メンション回数の降順（多い順）でソートします。
    4.  メンション回数が同じハンドルが複数ある場合、それらのハンドルはアルファベット順（昇順）でソートし、順位を確定させます。
