...
| `is_early_adopter` | `patrons` テーブルに存在 | 判定されるステータス | 説明 |
|---|---|---|---|
| `true` | `true` | `early_adopter_and_patron` | 早期採用者であり、かつパトロンであるユーザー。 |
| `true` | `false` | `early_adopter` | 早期採用者であるが、パトロンではないユーザー。 |
| `false` | `true` | `patron` | 早期採用者ではないが、パトロンであるユーザー。 |
| `false` | `false` | `normal` | 早期採用者でもパトロンでもない通常のユーザー。 |

この判定ロジックは、`User` モデルのアクセサ（例: `getStatusAttribute()`）として実装することで、`$user->status` のように簡潔にユーザーのステータスを取得できるようになります。これにより、ビューやコントローラーなど、アプリケーションの様々な場所でユーザーの特性に応じた処理を柔軟に記述できます。
