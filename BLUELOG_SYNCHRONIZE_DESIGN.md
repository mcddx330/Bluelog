...
#### 4.1.1. Laravelの `encrypted` キャストの活用

`users` テーブルに保存される `access_jwt` および `refresh_jwt` は、LaravelのEloquentモデルの `$casts` プロパティにおいて `encrypted` キャストが指定されています。
...
*   **自動的な暗号化・復号化:** データベースに保存する際にLaravelが自動的に値を暗号化し、データベースから値を取得する際に自動的に復号化します。これにより、開発者が手動で暗号化・復号化のロジックを記述する必要がなくなり、コードが簡潔になります。
*   **セキュリティの向上:** データベースが不正アクセスを受けた場合でも、機密情報が平文で保存されていないため、情報漏洩のリスクを大幅に低減できます。この処理は、Laravelの `Crypt` ファサードが透過的に実行しており、アプリケーションキー（`APP_KEY`）を用いた堅牢な暗号化が提供されます。

#### 4.1.2. `APP_KEY` に関する重要注意

`encrypted` キャストによるデータの暗号化・復号化は、Laravelのアプリケーションキー（`.env` ファイルの `APP_KEY`）に依存しています。

**重要注意項目:**

*   **一度設定した `APP_KEY` は本番環境では絶対に（特に暗号化されたデータが存在する状態で）変更してはいけません。**
*   `APP_KEY` を変更すると、それ以前にそのキーで暗号化された全てのデータ（`access_jwt` や `refresh_jwt` など）は**復号化できなくなり、読み取れなくなります。** これは、データが永久に失われることを意味します。

`APP_KEY` はアプリケーションのセキュリティの根幹をなすものであり、厳重に管理し、漏洩させないようにしてください。
