...
Bluelogのサービス稼働初期段階においては、システムの安定性と品質を確保するため、ユーザー登録を招待コード制としています。これは、段階的なユーザー増加を管理し、予期せぬ問題への対応を容易にするための戦略です。

**現状:**

現在のBluelogの実装では、招待コード制が導入されており、ユーザーはBlueskyアカウントでログインする際に招待コードの入力が必須です。招待コードは複数回利用可能であり、初期ユーザーへの特典付与や、招待コードの利用状況を詳細に把握するための機能も実装済みです。

**詳細:**

招待コードの生成、管理、検証ロジック、ユーザー登録フローへの組み込み、管理者向けの招待コード発行機能、招待コードの複数回利用と使用履歴の追跡機能、および招待コードを使用して登録したユーザーへの「早期採用者」フラグ付与については、`BLUELOG_USER_DESIGN.md` の「4.x. 招待コードの管理と利用 (新規)」セクションを参照してください。
...
*   **運用方法:**
    ...
    *   **使用回数制限:** 招待コードには任意の利用回数上限を設定できます。制限を設けない場合は複数回の利用も可能です。
