...
### 3. 利用者設定 (User Settings)

Bluelogの運用形態を管理者が柔軟に選択・設定できるようにするための機能です。これにより、Bluelogを個人利用のツールとして運用するか、あるいは招待制のコミュニティとして運用するかを決定できます。

**設定モード:**

*   **シングルユーザーモード:** 特定のBlueskyアカウントのみがBluelogを利用できるモードです。新規ユーザー登録は許可されず、指定されたアカウントのみがログイン・利用可能です。
    *   この時、初回ログイン（新規登録）を行ったBlueskyアカウントをBluelogの管理者として定義し、以降は新規登録処理が行われないようにする。（ログイン時にアカウント登録が1件でもあるかを確認）
*   **マルチユーザー（招待コード必須）モード:** 招待コードを用いて新規ユーザー登録を許可するモードです。このモードでは、既存の招待コード生成・管理機能が有効になり、招待コードを持つユーザーのみが登録・利用できます。

**注記:** 自由登録モード（招待コードなしでの登録）は、現時点では意図的に無効化されています。これは、サービスの安定性と品質確保のため、初期段階では招待コード制に限定するためです。

**永続化の仕組み:**

*   **`settings` テーブルへの追加:** アプリケーションの全体設定を管理する `settings` テーブルに、以下の新しい設定項目を追加します。
    *   `registration_mode` (string): アプリケーションの登録モードを示すキー。値は `single_user_only` または `invitation_required` のいずれかとなります。
    *   `allowed_single_user_did` (string, nullable): `registration_mode` が `single_user_only` の場合に、Bluelogの利用を許可する唯一のBlueskyアカウントのDIDを保存します。
*   これらの設定値はデータベースに保存され、アプリケーションのユーザー登録処理や、招待コード関連機能の表示・非表示を制御するために参照されます。設定値はキャッシュ機構を通じて高速に読み込まれます。

**UIの変更点:**

*   設定画面 (`resources/views/components/settings-main-content.blade.php`) に、管理者のみに表示される「利用者設定」セクションを追加します。
*   このセクション内に、上記2つのモードを選択するためのラジオボタンを配置します。
*   「シングルユーザーモード」選択時には、既存のBluelogユーザーのハンドル一覧から利用を許可するアカウントを選択するドロップダウンセレクターを表示します。
*   「マルチユーザー（招待コード必須）モード」選択時には、既存の「新しい招待コードを生成」セクション全体が表示され、招待コードの発行と管理が可能になります。
